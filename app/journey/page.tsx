...
Currently, I am working in a more demanding and rewarding role at Meissasoft. This is a full-fledged internship where I not only focus on coding and projects but also gain a deeper understanding of how the industry functions. I practice Object-Oriented Programming (OOP) by building a Chess Game Engine and a Ride Sharing Simulation. I solve Data Structures and Algorithms problems on HackerRank, design SQL databases, and create a CRUD-based Student Management System using FastAPI and SQLAlchemy. Unlike my short internship at InsightSol, this experience gives me real insight into the market—how companies structure projects, manage databases, build APIs, and test applications before deployment. Here, I truly feel like a part of the tech ecosystem rather than just a learner.`,
  ...
  return (
    <div className="min-h-screen dark:text-white text-midnight pt-32 pb-20 px-6 relative">
      {/* Main Container */}
      <div className="max-w-4xl mx-auto">
        ...
        >
          <h1 className="text-6xl md:text-7xl font-extrabold mb-6">
            My{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-lavender to-violet">
              Journey
            </span>
          </h1>
          <p className="text-xl opacity-70 max-w-2xl mx-auto dark:text-gray-400 text-midnight">
          ...
          {sections.map((section, index) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${section.color} opacity-0 group-hover:opacity-8 transition-opacity duration-500 rounded-3xl blur-sm`}
                />
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${section.color} opacity-0 group-hover:opacity-3 transition-opacity duration-500 rounded-3xl`}
                />
                ...
                <div className="relative z-10 flex items-center gap-4 mb-6">
                  <div className="p-4 rounded-2xl bg-lavender/20 border border-lavender/30 shadow-lg shadow-lavender/20 group-hover:shadow-lavender/40 transition-all">
                    <Icon
                      className="text-lavender group-hover:drop-shadow-lg transition-all"
                      size={28}
                    />
                  </div>
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-lavender group-hover:to-violet transition-all">
                      {section.title}
                    </h2>
                    {section.subtitle && (
                      <p className="text-sm opacity-60">{section.subtitle}</p>
                    )}
                  </div>
                </div>
