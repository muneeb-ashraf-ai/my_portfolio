    ...
    <>
      ...
      <section id="hero" className="min-h-screen flex items-center justify-center px-6 pt-20">
        <div className="max-w-7xl w-full grid md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-extrabold leading-tight mb-6 dark:text-white text-midnight">
              Hi, I'm <span className="text-lavender">Muneeb</span>... <br />
              <span
                className={
                  theme === 'dark'
                    ? 'text-transparent bg-clip-text bg-gradient-to-r from-lavender via-violet to-charcoal'
                    : 'text-lavender'
                }
              >
            ...
            <p className="text-lg md:text-xl opacity-80 max-w-lg mb-8 leading-relaxed dark:text-gray-300 text-midnight">
              ...
              <button
                ...
                className={`px-8 py-4 rounded-2xl font-bold border ${
                  theme === 'dark'
                    ...
                    : 'border-midnight/10 hover:bg-midnight/5'
                } transition-all`}
              >
                My Journey
              </button>
          ...
          >
            <div className="aspect-square relative z-10 rounded-[2.5rem] overflow-hidden border-8 border-lavender/10 group w-64 md:w-80 lg:w-96 mx-auto">
              <Image
                ...
                className={`object-cover transition-all duration-700 scale-100 group-hover:opacity-100 ${
      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            >
              <h2 className="text-4xl md:text-5xl font-extrabold mb-2 dark:text-lavender text-lavender">
                {stat.val}
              </h2>
              <p className="text-sm opacity-60 font-medium uppercase tracking-widest dark:text-gray-400 text-midnight">
                {stat.label}
              </p>
            </motion.div>
          ))}
      ...
      <section id="projects" className="py-32 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
            <div>
              <h2 className="text-4xl md:text-5xl font-bold mb-4 dark:text-white text-midnight">
                Selected <span className="text-lavender">Work</span>
              </h2>
              <p className="opacity-60 max-w-md dark:text-gray-400 text-midnight">
                A collection of projects where I've applied AI and Data Science solutions.
              </p>
            </div>
            <button
              onClick={() => router.push('/projects')}
              className="text-lavender font-bold flex items-center gap-2 group hover:gap-3 transition-all"
            >
            ...
            {PROJECTS.map((project, i) => (
              <motion.div
                ...
                className={`group rounded-3xl overflow-hidden ${
                  theme === 'dark'
                    ? 'glass-morphism hover:shadow-xl hover:shadow-lavender/30'
                    : 'glass-morphism-light hover:shadow-xl hover:shadow-lavender/20'
                ...
                <div className="p-8">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-[10px] font-bold px-2 py-1 rounded-full bg-lavender/10 text-lavender uppercase tracking-wider group-hover:bg-lavender group-hover:text-white transition-all"
                      >
                        {tag}
                      </span>
                    ))}
                  </div>
                  <h3 className="text-2xl font-bold mb-3 group-hover:text-lavender transition-colors">
                    {project.title}
                  </h3>
                  <p className="text-sm opacity-70 mb-6 leading-relaxed group-hover:opacity-90 transition-opacity">
                    {project.description}
                  </p>
      ...
      <section id="services" className="py-32 px-6 relative">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-5xl font-bold mb-8 dark:text-white text-midnight">
              Elevating AI & <br />{' '}
              <span
                className={
                  theme === 'dark'
            ...
            <div className="space-y-8">
              {[
                {
                  icon: (
                    <Code className="text-lavender group-hover:drop-shadow-lg transition-all" />
                  ),
                ...
                {
                  icon: (
                    <Zap className="text-lavender group-hover:drop-shadow-lg transition-all" />
                  ),
                ...
                {
                  icon: (
                    <Palette className="text-lavender group-hover:drop-shadow-lg transition-all" />
                  ),
          ...
          >
            <h3 className="text-3xl font-bold mb-8 dark:text-white text-midnight">
              What I Bring
            </h3>
      ...
      <section id="contact" className="py-32 px-6">
        <div
          className={`max-w-7xl mx-auto rounded-[4rem] p-12 md:p-24 text-center relative overflow-hidden ${
        ...
        >
          <div className="relative z-10">
            ...
            <p className="text-lg opacity-70 max-w-2xl mx-auto mb-10">
              Have a project in mind or want to discuss opportunities? Reach out and let's build something impactful together.
            </p>
            <button
              onClick={() => router.push('/contact')}
              className="px-10 py-4 bg-lavender text-white rounded-2xl font-bold shadow-xl shadow-lavender/40 hover:scale-105 transition-transform"
            >
              Contact Me
            </button>
          </div>
          <div className="absolute -top-10 -left-10 w-32 h-32 bg-lavender/20 rounded-full blur-3xl" />
          <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-violet/20 rounded-full blur-3xl" />
        </div>
      </section>
    </>
  );
